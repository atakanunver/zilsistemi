...
## 2026-09-18 - Etkinlik (kermes/festival) YouTube çalma özelliği + kritik YouTube altyapı düzeltmeleri

- Kullanıcı isteği: kendi YouTube playlist'ini dashboard'dan link yapıştırıp direkt hoparlörden çaldırabilsin (kermes/festival gibi etkinlikler için) - playlist/ klasörüne/tenefüs otomasyonuna hiç karışmadan.
- dashboard.py <-> sys.py arasında yeni dosya-tabanlı komut kanalı: oynatma_istegi.json (dashboard yazar) + oynatma_durumu.json (sys.py yazar, dashboard polling'ler). sys.py'de yeni etkinlik_dinleyici() async task'ı (3sn'de bir kontrol), /youtube-cal sayfası (link yapıştır+çal+durdur+canlı durum).
- KRİTİK bulgu (canlı testte ortaya çıktı, muhtemelen önceden de var olan bir production bug'ıydı): VLC bu sunucudan YouTube CDN'ine (googlevideo.com) DOĞRUDAN TLS bağlantısı kuramıyor (okul LAN güvenlik duvarı, Connection reset by peer/Network unreachable - sertifika sorunu değil, bağlantı engeli). Bu, mevcut youtube_link_handler (Telegram link yapıştırma) ve duyuru_otomasyonu'nun YouTube kolunu da etkiliyordu - muhtemelen sessizce bozuktu, hiçbir yerde loglanmadı (hata sadece Telegram mesajına yazılıyordu).
- Çözüm: artık HİÇBİR yerde VLC'ye ham CDN akış URL'si verilmiyor. youtube_ses_url_al() -> youtube_indir() olarak değişti: yt-dlp ile önce YOUTUBE_CACHE_DIR'e (geçici, gitignore'da) indirilip VLC YEREL dosyayı çalıyor (playlist mp3'leriyle aynı yöntem). Üç çağrı noktası da (youtube_link_handler, duyuru_otomasyonu, _etkinlik_parca_cal) güncellendi. Tek slotluk temizlik: her yeni indirmeden önce bir önceki geçici dosya silinir.
- AYRI bulgu: yt-dlp 2026.3.17 (eski) YouTube'un bot-korumasına takılıp '403 Forbidden' veriyordu (indirme adımında - metadata çözme çalışıyordu). Çerez VE Müdür PC proxy'si VE farklı player_client seçenekleri (ios/web/tv/web_safari) hiçbiri çözmedi - 2026.8.19'a yükseltmek çözdü. yt-dlp sürümü güncel tutulmalı, bu tarz bot-koruma kırılmaları sık sık yaşanıyor.
- GÜVENLİK notu: kullanıcı sorun giderme sırasında YouTube/Google hesabının TAM oturum çerezlerini (SID/HSID/SSID/LOGIN_INFO vb, sadece YouTube değil tüm Google hesabına erişim sağlayan) paylaştı - bunlar sunucuya yazılıp test edildi (fayda sağlamadı, farklı bir hataya yol açtı) sonra kalıcı olarak silindi. Gerçek çözüm yt-dlp güncellemesiydi, çereze hiç gerek yoktu.
